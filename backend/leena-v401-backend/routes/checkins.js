...
router.post('/', authenticateToken, async (req, res) => {
  ...
  // Only accept check-ins from badge-print terminal
  if (terminal !== 'badge-print') {
    return res.status(400).json({ 
      error: 'Invalid terminal',
      message: 'Check-ins are only accepted from badge-print terminal'
    });
  }
    ...
    if (visitor.last_checkin) {
      ...
      if (timeSinceLastCheckin < minimumInterval && checkin_type === 'entry') {
        ...
        return res.status(409).json({ 
          ...
          message: 'This visitor was checked in less than 2 minutes ago',
