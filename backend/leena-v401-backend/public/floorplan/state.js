...
class FloorPlanState {
  ...
  selectStand(stand) {
    ...
    this.selectedStands = stand ? [stand] : [];
    this.emit('standSelected', stand);
  }

  toggleStandSelection(stand) {
    if (!stand) return;
    const idx = this.selectedStands.findIndex(s => s.id === stand.id);
    if (idx !== -1) {
      this.selectedStands.splice(idx, 1);
    } else {
      this.selectedStands.push(stand);
    }
