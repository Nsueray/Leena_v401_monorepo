...
| G43 | **Render Health Check Path = `/health` cuts the 502 window to ~20 s.** Before the health-check wiring: cutover windows of 22-45 s (sometimes worse — May's MP26 push saw 50 s). After: measured 20 s (17:12:28 → 17:12:48 UTC+3 on the 7 Sep post-launch push, first-502 to last-502). Why: Render waits for `/health` to return 200 on the new container before switching traffic; the old container drains through the same probe until the new one is ready. Cutover happens only when the new process is genuinely ready (module load + first-query cost) — nothing before that ever hits a cold container. Doesn't eliminate the 502 window, just narrows it to the actual switchover moment. Combined with G42, deploy cost is now bounded. |
