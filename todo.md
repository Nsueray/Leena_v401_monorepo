...
| **BACKEND (call-center, before agents work full-day)** | **Wire the daily-report cron.** `POST /api/callcenter/report/send-now` works via the admin page (JWT). External schedule fires the same endpoint at 19:00 Casablanca via Render Cron / GitHub Action / Suer's cron. Design's R9 minimum was "manual button + external cron"; the button is live, the cron is not. |
| **BACKEND (before SIEMA campaign day 2, tomorrow)** | **Stats "Registered" audit.** Campaign detail page's "Registered" funnel count is derived from `campaign_events.event_type='registered'` — need to confirm it matches the raw `reactivation_tokens.status='activated'` count. Today's launch showed **704 activations** end-of-day but the funnel figure may lag / drift. Simple compare query for expo 9 tomorrow morning: `SELECT (SELECT COUNT(*) FROM reactivation_tokens WHERE target_expo_id=9 AND status='activated') AS token_actv, (SELECT COUNT(DISTINCT recipient_id) FROM email_events WHERE campaign_id IN (78,79) AND event_type='registered') AS event_reg;` — characterise any drift + write into the deploy doc. |
...
- [x] Thank-you page feature (Job B Option A) DEPLOYED 5 Sep via merge commit `4d0263d`. Live QA passed on form 51 (English byte-identical) and form 59 (Yaprak set `config.language='fr'` → full French card renders end-to-end). Full narrative + config keys + resolution order: `docs/sessions/DEPLOY_THANKYOU_PAGE_20260905.md`.

---

## ✅ CLOSED — 7 Sep (SIEMA launch day + call-center)

- [x] **SIEMA26 launched.** Campaigns 78 (activate) started 09:00 Casablanca, 79 (register) 11:00. Send rate ~240/min. 236 recipients skipped by step-1 `not_registered` — the load-bearing 4 Sep fix delivered. 15:00 funnel: 16,236 sent · 2,941 opened · 1,134 clicked · 618 activated (grew to 704 EOD). Full narrative: `docs/sessions/SIEMA26_LAUNCH_DAY_20260907.md`.
- [x] **Call-center dialer module** — isolated new module shipped in 5 commits. One new table (`callcenter_leads`, migration 030) + two partial indexes on `email_queue` (migration 031). Two auth middlewares (`callCenterAgentAuth`, `callCenterSupervisorAuth`) + one route file (`routes/callcenter.js`, ~1,160 lines, 10 endpoints) + three pages (`public/callcenter/{agent,supervisor,admin}.html`, ~1,850 lines). Real import 33,635 leads across segments A/B/C (15,016 / 2,727 / 15,892). Yaprak smoke uncovered the ccFetch 400 splash-drop bug (Stage 4c-a fix, new gotcha G45) + segment pill copy needed plain-language (Stage 4c-b, EN/FR/TR pill_A/B/C). Stage 4c-c added one-line hint under every action. Stage 4d normalised agent name (trim/collapse/uppercase) + optional `CALLCENTER_AGENTS` allowlist. Full doc: `docs/sessions/DEPLOY_CALLCENTER_20260907.md`.
- [x] **`last_name` required on activation pages + endpoint** — 61 of 685 activations were landing with empty last_name (9%), 9,376 of 15,788 pending tokens have no last_name at mint. Three-layer fix: `required` + `oninvalid` inline `<div>` (`Last name required` / `Nom obligatoire`) + submit-handler belt-and-braces + server 400 `LAST_NAME_REQUIRED`. Prefill unchanged. Full doc: `docs/sessions/DEPLOY_LAST_NAME_REQUIRED_20260907.md`.
- [x] **Sender display name on transactional mails** (a follow-up from 6 Sep, commit `aed7898`) — worker's `fromName` no longer gated on `isCampaignTask`. Every transactional send (badge/QR mail, per-form sales notification, Excel-import badge, bulk-email panel, conference certificates, reactivation flows, Zoho webhook, segments bulk) now ships with "Elan Expo <noreply@leena.app>" instead of bare `noreply@leena.app`. Peer helper `_getExpoNameByExpoId` mirrors the existing campaign cache.
- [x] **Smoke-rows investigation on `callcenter_leads`** — 5 rows disappeared mid-day; read-only investigation exonerated all code paths + deploy path (no `DELETE FROM callcenter_leads` anywhere in repo, no startup migration runner, `initial.sql` has zero references to the table). Cause: design-doc cleanup SQL was run twice. First run silently removed 5 rows; second returned 0. New gotcha `G44` — smoke cleanup SQL must use `DELETE ... RETURNING id` + per-session source tag. Verdict: real import is safe, no deploy-wipe risk.
- [x] **G42 — deploy-freeze rule on campaign send days.** Three daytime deploys (12:22/12:47/13:58 IST) caused a visitor to report "site inaccessible" mid-activation-click. New rule: no deploys 07:00–22:00 Casablanca on any live-send day.
- [x] **G43 — Render Health Check Path = `/health` cut the 502 window to ~20 s.** Measured 17:12:28 → 17:12:48 UTC+3 on tonight's push. Combined with G42, deploy cost is now bounded.
- [x] **G45 — 400 is not an auth error** (call-center ccFetch bug + rule).
...
| 19 | **"Create New Edition" — clone-expo flow** | P2 | Clone an expo: copy forms, email templates, badge designs, settings, cluster; DO NOT copy visitors. Add `previous_edition_id INTEGER REFERENCES expos(id)` link. Picker cards then show `±% vs previous edition` using the new `registered_count` (commit `4b69814`). Enables spinning SIEMA 27 from SIEMA 26 in one click without losing lineage. |
| 20 | **`GET /api/expos/:id` — add `registered_count` + `unique_checkin_count`** | P2 | Follow-up to commit `4b69814`. Detail endpoint at `routes/expos.js:114-115` still returns only `visitor_count` + `checkin_count`. Feeds `expo-form.html:289`; also a future-edit-panel candidate. Same two subqueries as the list endpoint. |
| 21 | **`checkin-import.html:78-79` help text lies** | P3 | Text claims "Required: badge_id or email" but `routes/import-checkins.js:88` parser only accepts `visitorId` and throws on anything else. Fix the help text OR extend the parser. Cosmetic today. |
| 22 | **Edit Expo UI absent — backend PUT ready** | P2 | `PUT /api/expos/:id` at `routes/expos.js:282` has the full field whitelist and works. Only legacy `dashboard.html` (deprecated) carries an `editExpo()` handler; active `dashboard_new.html` has no edit modal. Design + ship a matching edit modal (mirror the Create modal from commit `8907612` — same country select, same date pair). |
| 23 | **`main-panel-v2` stat semantics — align with expo picker** | P2 | The main expo dashboard reads `total_visitors` from `/api/reports/summary`, a different endpoint from the expo picker. After picker rewrite (`4b69814`) the two pages can disagree on the same expo. Align semantics — either summary uses same origin filter, or picker's numbers propagate. |
| 24 | **Archive/hide test expos (4, 11, 15, 16, 17)** | P3 | They clutter the picker and confuse Yaprak. Add `is_archived BOOLEAN` column + toggle in Edit modal, OR a naming-convention filter (`[TEST]` prefix hidden by default with an "Include test expos" checkbox). Post-fair, low priority. |
| 25 | **DROP `visitors_badge_backup_20260909`** | P3 | 2,869 rows, kept for rollback of the 9 Sep expo-9 badge cleanup (commit `[docs]` covers). Safe to drop any time after 24 Sep. |
| 26 | **Campaign Wizard: from-expo source filters** | P2 | Add filters to the from-expo source: all / checked-in only / registered-no-show / by source label (e.g. `madesign2025_checkin`). Removes the need to cut Excel files for campaigns whose audience already lives in the DB. **Caution:** from-expo path has never run live (all runs incl. SIEMA were Excel-sourced); ~50 LoC but requires smoke test on expo 17 first. No daytime deploys until SIEMA drips end (G42). **Ref:** Madesign 2026 prep, 9 Sep — decision "option 1 now, ship filter later". Post-Madesign, early Oct. |
| 27 | **`routes/emailSend.js` `/single` + `/bulk` — add `badge_link` to `emailData`** | P2 | Panel currently builds `emailData` at `:99-108` (`/single`) and `:238-249` (`/bulk`) WITHOUT `badge_link`. **RULE UNTIL FIXED: templates 28/47/69/86/87/88 must NOT be sent from the Send Emails admin panel** — they carry the new QR fallback line as `<a href="{{badge_link}}">…</a>`, and `emailSend.js` would render that as `href=""` (dead visible link, worse than no link). Fix is ~4 lines: derive `badgeLink` from `qrCode` next to the existing `qrImageTag` derivation, add to `emailData`. Then this rule expires and future QR templates can add the fallback safely. Post-fair. |
| 28 | **CID inline QR images — permanent corporate-gateway fix** | P2 | The `{{badge_link}}` fallback shipped 10 Sep is a workaround; the permanent fix for Mailinblack/Barracuda/Exchange gateways is CID (`content_id`) inline attachments. Requires `sendEmail`/`sendEmailWithReplyTo` in `utils/email.js` to accept an `attachments` param (SendGrid API supports `disposition:'inline'` + `content_id`), plus template edits changing `{{qr_code}}` from external `<img src>` to `<img src="cid:qrcode-cid">`. Generates the QR PNG server-side per send (~2 KB). Full design: `SIEMA_OPS_20260910.md §3` + 10 Sep root-cause Issue A option (c). Post-fair. |
| 29 | **`/api/qr-image` — 404 on unknown UUID + cache headers** | P3 | `index.js:183-195` encodes ANY input string via `QRCode.toBuffer` and always returns 200. Junk UUID → returns a valid PNG that scans to junk. Should validate the qr_code against `visitors.qr_code` and 404 on miss. Also add `Cache-Control` + `ETag` to reduce email-client re-fetches (some gateways interpret cache-headerless as "fetch every open" → rate-limit). Post-fair. |
| 30 | **Bulk import undo button** | P2 | 10 Sep: 149 Nigeria exhibitor rows landed on expo 9 (wrong dropdown), caught same day, cleaned via `visitors_import_mistake_backup_20260910` + manual DELETE. Zero emails triggered because `send_email` was off. **Add an "Undo last import" button on `import.html`** that (a) is available for N minutes after import completes, (b) refuses if ANY row from the import has been mailed OR appeared in a check-in, (c) DROPs the rows + related email_queue in one transaction. Post-fair. |
| 31 | **External uptime monitor on `/health`** | P2 | 9 Sep 20:53-21:29 UTC outage was **human-detected** — Suer noticed via a visitor complaint. UptimeRobot / Better Uptime / Pingdom, 1-min interval, alert Suer via SMS+email at ≥2 consecutive failures. G43 upper bound is 60s; anything past 90s indicates a real problem, not a normal cutover. Post-fair. |
| 32 | **Inbound reply loop — staff replies to forwards may re-enter Inbound Parse** | P3 | G48 fixed the raw-MIME parser bug (SendGrid checkbox flipped 10 Sep ~13:20 UTC). Latent question: when Suer/Yaprak reply to a `[Visitor Reply]` forwarded email, does that reply also go through `reply@replies.leena.app` and get re-forwarded (cosmetic duplicate)? `routes/emailInbound.js` has no From-address filter. Evaluate by inspecting one such reply after the first post-fix sample arrives; if it loops, add a From-domain filter. Post-fair — cosmetic only, not lost data. |
| 33 | **Patch audit trail — who edited which lead field** | P2 | 11 Sep: no way to see who changed what on a `callcenter_leads` row (agent's `fix_data` block edits email/phone/name/company/country in place; edits are invisible in retrospect). Add a `callcenter_lead_edits` table (`id, lead_id, agent, field, old_value, new_value, edited_at`) written by the /outcome route when any of the 5 patchable fields differ from the SELECT-loaded value. Small — one INSERT per changed field per /outcome. Post-fair. |
| 34 | **CALLCENTER_AGENTS allowlist activation** | P2 | Env var + middleware guard shipped Stage 4d (commit `1f8692a`, 7 Sep). BLOCKED: needs Meriem's definitive agent name list (post-normalisation form: trim → collapse ws → uppercase). Once given, set `CALLCENTER_AGENTS=NAME1,NAME2,...` on Render web service → all `agent` names outside the list are rejected with `400 AGENT_NOT_ALLOWED`. **Not code — coordination.** Post-fair unless SIEMA week 2 team-list stabilises earlier. |
| 35 | **Uptime monitor scope — extend to Natro landings** | P2 | Item 31 (external uptime on `/health`) needs to also cover `siemaexpomorocco.com` and `madesignmaroc.com` (Natro hosting). 11 Sep 00:10-11:00 IST Natro outage cost ~168 zoho registrations — Suer noticed via user complaint at 11:00; a monitor would have alerted at 00:15. Same UptimeRobot/Better Uptime setup, 3 monitored URLs instead of 1. Post-fair. |
| 36 | **Structured `not_interested` sub-reasons** | P3 | Item 45c8d61 (11 Sep) made the note field required when outcome=`not_interested`, but the note is free-text. Post-fair: add a sub-reason dropdown (wrong_sector / not_available / out_of_town / language_barrier / no_time / other) that replaces or complements the note textarea. Enables supervisor-side quantification of "why do people say no" without free-text grep. |
| 37 | **Meta-ads direct-to-Leena fallback option** | P2 | Marketing decision, not code — 11 Sep Natro landing outage cost ~168 registrations because Meta ads all route through `siemaexpomorocco.com` / `madesignmaroc.com`. Alternative: point some fraction of ad spend directly to `leena.app/form-public.html?id=59` (Leena is on Render, independent infra). Trade-off: loses the Natro landing's marketing copy + branding. Discuss with Yaprak/marketing team post-fair. |
| 38 | **DROP `email_templates_backup_20260916`** | P3 | 3 rows — pre-chain snapshot of templates 86/87/88, taken 16 Sep before the `{{badge_link}}` → `{{badge_link\|badge_url}}` rewrite. Rollback = column-wise restore of `html_content`/`body` by `id`. **Safe to drop after 5 Oct 2026** (Madesign closes 3 Oct). Full record: `docs/sessions/MANUAL_DB_CHANGES_20260916.md §4`. |
| 39 | **FR country + sector option lists on form 64** | P2 | `forms.fields` for form 64 still stores **268 English country names** (`Ghana, Åland Islands, Afghanistan, Akrotiri, …` — note `Ghana` is pinned first, out of alphabetical order) and **30 English sector options** (`Manufacturer / Producer, Distributor, Wholesaler, …`) on an otherwise fully French form. Visitor-facing. 16 Sep audit #4/#5 — left unfixed during the launch window by decision. Needs Yaprak to supply the FR lists, then a `jsonb_set` on `fields[7].options` / `fields[8].options`. Consider a shared reference list rather than per-form option arrays. |
| 40 | **Hardcoded EN strings in `form-public.html`** | P3 | Five visitor-facing strings bypass the `config.language` switch because they are hardcoded rather than routed through the `pickSuccess()`/`pickError()`/`pickMisc()` defaults: `:222` `Submit Registration` (pre-style default), `:338` `Submitting...`, `:203` `Failed to load form. Please try again later.`, `:399` `Failed to submit form. Please try again.`, `:496` QR `alt="Your entrance QR code"`. On form 64 the first is masked by `config.style.buttonText`, but the other four still render EN on a FR form. Fix = add FR entries to the `MISC_DEFAULTS_*` maps (`:455-460`) and route these through `pickMisc()`. 16 Sep audit #9. |
| 41 | **`(Resent)` subject suffix is English-only** | P3 | `routes/visitors.js:406` appends the literal `(Resent)` when a returning visitor re-submits a form, producing `Votre inscription est confirmée – Votre code QR est prêt (Resent)` on the French Madesign template. Fix = language-aware suffix (form's `config.language`, or drop the suffix and rely on the timestamp). 16 Sep audit #10. |
| 42 | **English server-side reactivation errors surface on the FR page** | P3 | `routes/reactivation.js:622-690` returns `Invalid or expired token`, `This activation link has expired`, `This visitor pass has already been activated`, `Last name is required` — rendered verbatim by `reactivate-fr.html:588`/`:601`. Known and accepted (documented in `reactivation-campaign.html:444`: "Server-side error messages remain in English regardless"), but a French visitor hitting an expired link sees English. Fix = return a code instead of prose and let the page map it, or accept permanently and delete the caveat. 16 Sep audit #11. |
| 43 | **`forms.submission_count` is stale** | P3 | Form 64 reads `submission_count = 0` while `visitors WHERE form_id=64` returns **661** rows (09-09 → 09-15). The counter is not maintained by the public-form POST path. Either wire an increment into `routes/visitors.js` POST `/public`, or drop the column and derive the count. Cosmetic today — confirm nothing on a dashboard reads it before deciding. 16 Sep audit #12. |
| 44 | **post-fair: native `badge_link` on the activation path** | P2 | Add `badge_link` to `templateData` at `routes/reactivation.js:853-865`, mirroring `routes/visitors.js:384` (`` `${baseUrl}/badge-print.html?qr=${qrCode}` ``). The `{{badge_link\|badge_url}}` chain shipped in templates 86/87/88 on 16 Sep is a **measured-complete** fix (resolves via `badge_url` on this path), so this is hygiene, not a defect — it removes the reliance on template-side chaining for future templates. **The chain stays in 86/87/88 as insurance after this lands.** Deferred out of the launch window by decision; full rationale in `docs/sessions/MANUAL_DB_CHANGES_20260916.md §3` + `§7`. |
...
- **P2 — Footer postal address should come from the expo's organiser entity, not a hardcoded constant** — one address per country office. Today (2 Sep, footer commit) the Morocco address `ELAN EXPO MAROC SARL / 30, Bd Rahal El Meskini, 2ème Etage, Appart N° 5, Casablanca, Morocco / +212 650 219 756` is hardcoded in `utils/trackingPixel.js` `injectUnsubscribeLink` for the SIEMA launch. Correct for SIEMA (Morocco) — WRONG for the next Nigeria / Ghana / Kenya campaign on the same code, which will show a Casablanca postal address for a Lagos event. Design: schema column on `organizers` (or a new `organizer_offices` table keyed by country) → derive at send time from `campaign.expo_id → expos.country_code → office row`. Must land before the next non-Morocco campaign uses `enqueueStepEmail`.
- **P2 — Per-campaign From display name via schema column, not env var.** Today (3 Sep, SIEMA pre-launch) `CAMPAIGN_SENDER_NAME` env var on the Render worker service applies to ALL active campaigns until unset. Single global name works for SIEMA (no concurrent campaigns for other fairs). Trigger: the first time two campaigns for different fairs need distinct display names concurrently. Design: `email_campaigns.from_display_name VARCHAR(255) NULL` column + wizard Confirm-panel field + `email_worker.js` reads the row column instead of the env var (env var stays as fallback). Change is scoped to one column + one wizard field + one `enqueueStepEmail`-adjacent query. Reference: check performed 3 Sep, `utils/email.js:64+` + `email_worker.js:229-241` diff.
- **P2 — Cross-campaign overlap scope should widen to "any campaign of this organizer"** when the multi-expo drip pattern emerges. Today (3 Sep, commit `72a29d4`) the wizard's 5th prefetch query in `routes/campaignBuilder.js` filters `ec.expo_id = $1` — target-expo-scoped by design. Correct for the current pattern (two campaigns on two different expos are two different events, not a duplicate). Trigger: the first time Yaprak builds a fair-agnostic newsletter that spans multiple expos, or any pattern where a person legitimately receives Elan-Expo-wide mail across expos. When that happens: remove the `ec.expo_id = $1` predicate and update the amber warning copy from *"another campaign for this expo"* to *"another campaign from you"* (or similar). See `docs/sessions/DEPLOY_CROSS_CAMPAIGN_DEDUP_20260903.md` §9.1.

- **P2 — MillionVerifier API integration (verify once, store on visitor).** Manual CSV round-trip per campaign (upload → wait → download "Good" subset → import) is the current pattern. Wire the MV API to run per-row at import time (visitor create/upsert paths in `routes/visitors.js` `/import`, `/manual`, `/public`, and `routes/webhook.js` Zoho handler), cache the verdict on a new `visitors.email_verified_at` + `email_verified_reason` column, respect the cache on subsequent imports (dedupe by `lower(trim(email))` + skip re-verify within N days). Trigger: Yaprak wants to stop doing the MV round-trip by hand for every campaign. Manual pattern captured in `docs/sessions/SIEMA26_LAUNCH_RUNBOOK.md §1`. **Rate-limit / cost caveat:** MV charges per verification — the cache is load-bearing, not optional.
...
- **P2 — Call-center Stage 6: `form-public.html` field prefill from URL params.** Call-center's "Register now" button on the agent card currently opens a blank form 59; agent has to retype name/phone/email that's already on the lead. Design says ~8 lines in `form-public.html:234+` `renderField()` — read `URLSearchParams` and inject as the initial `value` of each `<input>`. Then the call-center agent page builds a URL like `form-public.html?id=59&name=X&last_name=Y&phone=+212...&email=Z`. **Deploy after 24 Sep** — `form-public.html` serves live SIEMA traffic and any change here touches the primary registration UI. Byte-identical for forms that don't pass query params.

- **P2 — `reactivate.html` / `reactivate-fr.html` should render form 59's field configuration.** Today the activation page carries a hardcoded 7-field block (`name`, `last_name`, `email`, `company`, `country`, `job_title`, `phone`). Form 59 has its own `fields` JSONB with the canonical shape (e.g. `hear_about_event`, `business_represent`, `main_reason_for_visiting`). The two are out of sync — an activated visitor's row doesn't get the same shape as a public-form-registered visitor's row. Suer's request 7 Sep: activation page should read `forms.fields` for form 59 (or whatever form the token's `form_id` points to) and render dynamically, matching what a public form submission would produce. **Post-fair** — sizeable UI change, touches live activation traffic.

- **P2 — Call-center `registered_meanwhile` stats audit.** `/next`'s auto-close path (`routes/callcenter.js:200+`) appends a `note` when a lead is closed as `registered_meanwhile` but doesn't emit a separate metric for the supervisor page. Confirm the supervisor's "Reg. after" column counts these auto-closes correctly vs. agent-selected `registered_meanwhile` outcomes. If wrong, split into two counters ("auto-closed by /next" vs "agent picked registered_meanwhile").

- ~~**Thank-you page feature — Saturday-morning deploy.**~~ ✅ **DEPLOYED 5 Sep** — merge commit `4d0263d`. Live QA passed on form 51 (English byte-identical) and form 59 (`config.language='fr'` set by Yaprak → French card renders end-to-end). Full narrative: `docs/sessions/DEPLOY_THANKYOU_PAGE_20260905.md`. New gotcha `G41` documents the resolution order + editable keys + `<html lang>` contract. Branch `feat/thankyou-page` retained locally (safe to delete on next cleanup pass).
...
- [x] **Wizard holdout at build time (control group, opt-in, 0–20%)** — commit `77297e7`. Fisher-Yates selection in request-path scope (captured in `setImmediate` closure) so Phase 2 mint filter skips held-out emails BEFORE any `reactivation_tokens` row is minted. Rows still land in `campaign_recipients` as `status='holdout'` (VARCHAR(30), no CHECK) so `/lift` can count them; `next_step_due_at` stays NULL so both send-path filters (`campaigns.js:917-918`, `email_worker.js:347-348`) miss them; `checkCampaignCompletion` (`email_worker.js:671-674`) uses `NOT EXISTS(status='active')` so holdouts don't hold the campaign open forever. Cross-campaign overlap query updated to `cr.status IN ('active','holdout')` — without this a second campaign would silently mail Campaign A's control group and contaminate lift. Zero-holdout drift guard: when `holdout_pct` is 0/absent, the two Sets are empty → byte-identical to prior behaviour (STEP 8's assertion still passes). Confirm-panel `#wHoldoutPct` (0–20) + live `wUpdateHoldoutSummary()` client math matching backend `round()`. Verified via STEP 9 (holdout_pct=20 on 5-row fixture → 1 G2 holdout, exact DB shape asserted). Full narrative: `docs/sessions/DEPLOY_SIEMA_BATCH_20260903.md §2 (77297e7)`.

- [x] **Wizard `/lift` report + one-line lift on Stats tab (Delivery B)** — commit `f6e7f44`. New read-only `GET /api/campaigns/:id/lift` behind `authMiddleware`, organizer-scoped via `WHERE ec.organizer_id = $2`. Bucket by `CASE WHEN cr.status='holdout' THEN 'holdout' ELSE 'mailed' END`, LATERAL-count registrations + check-ins per bucket via email-JOIN (`campaign_recipients.visitor_id` is 0% populated on excel uploads). Route mount verified: `campaigns.js` has no `/:id/lift` handler, so Express falls through to `campaignBuilder` at `index.js:146`. Stats tab placeholder `#wLiftLine` injected in `renderStats`, `wLiftFetch(campaignId)` fire-and-forget at end. Copy verbatim per Suer's spec ("Mailed: X% registered · Holdout: Y% · Lift +Z pts" + pre-fair/post-fair sub-lines). Silent no-op paths: pre-holdout campaigns (`holdout=null`), pre-lift-endpoint backends (404), network errors — Stats tab byte-identical to before. Zero touches to Yaprak's daily flow. Verified via STEP 10 against activateCamp9Id from STEP 9 (2 mailed / 1 holdout, both `rate_pct=0`). `docs/WIZARD_USER_GUIDE.md` (NEW, +119). Full narrative: `docs/sessions/DEPLOY_SIEMA_BATCH_20260903.md §2 (f6e7f44)`.

- [x] **Wizard SIEMA batch — sender name, French page, form design, phone through resolveCountry+normalize, French footer** — commit `52cc517`. Five items + Change B + Small Add, single deploy. **Sender display name** via optional `fromName` param on `sendEmailWithReplyTo` → SendGrid `{email, name}` → *"Elan Expo <noreply@leena.app>"* (env `CAMPAIGN_SENDER_NAME` set on Render worker service 3 Sep, covers Madesign 1 Oct too). **French activation page** `reactivate-fr.html` (NEW, +641, byte-for-byte copy of `reactivate.html` + 24 strings translated + French date locale) selected via `/build`'s `activation_lang: 'fr'` → Phase 3 URL builder at `campaignBuilder.js:1007`. **Activation form design** via `/build`'s `form_id` (shape + ownership check, threaded to `processReactivationChunks` replacing the hardcoded `null` from SIEMA prelaunch audit §6b). **Phone Decision B**: Excel reader applies `resolveCountry(row.country, targetCountryCode, map)` FIRST, then `normalizePhone(raw, resolved)` — France "06…" on MA target → +33 (not +212); row country wins over target. **French footer sentence** in `injectUnsubscribeLink` when `expoCountryCode === 'MA'` (strict upper-case; NULL / undefined / '' / other → English fallback; never throws). `expoCountryCode` threaded through `processCampaign` → `processRecipient` → `enqueueStepEmail` → `injectUnsubscribeLink` (verified: every call site is the new arity, no legacy). **Small Add**: `/segment` response includes `target_country_code`, UI defaults language select (`'MA'` → Français preselected, sticky via `dataset.userTouched`). Full narrative: `docs/sessions/DEPLOY_SIEMA_BATCH_20260903.md §2 (52cc517)`.

- [x] **Wizard Phase 2b — propagate `form_id` + `phone` to reused pending tokens** — commit `7e34f9e`. Live 4-recipient test on production expo 9 (~16:32 UTC) exposed that when Phase 2 REUSES existing pending tokens (`_hasToken=true`, Phase 3 SELECT at `:1078` pulls them alongside fresh mints), the tokens kept their original `form_id` (usually NULL → default yellow theme) and original `phone` (usually empty). Confirm-panel choices (form 59 + row phone) had zero effect on repeat runs. New Phase 2b block at `campaignBuilder.js:1057-1116` inserts between Phase 2's mint call and Phase 3's SELECT: filter set = `!_unsub && _hasToken && !S_holdoutG2.has(email)`, chunked (1000/statement) batch UPDATE with VALUES join. `form_id` → always to `formIdForActivation` (may be NULL); `phone` → COALESCE guard (empty-only, never clobber a good phone); `status='pending'` gate (never touch activated/expired); `updated_at` NOT SET (verified read-only: `reactivation_tokens` has no `updated_at` column — 19 columns; not added). STEP 6 extension: probe form created on `TEST_EXPO_ID` (`POST /api/forms`), pre/post snapshots assert `form_id` moves from NULL → `probeFormId` for all 3 reused tokens AND phones byte-equal (COALESCE guard held). Verified live via campaign 60 on expo 9: 4 tokens minted at 16:32:49, all `form_id=59`, phones `+212661234567/8/9` + `+33612345678` (Decision B proven). Full narrative: `docs/sessions/DEPLOY_SIEMA_BATCH_20260903.md §2 (7e34f9e)` + `§3 live evidence`.

- [x] **Wizard step-column headers, tooltips, step-1 disabled + backend `normaliseStep1`** — commit `81c9a0f` (2 Sep 18:45, folded into this batch). Header row + `title=` tooltips + row-1 inputs `disabled` + array mutated on every render to force `delay_hours=0 / condition='all'` (matches `campaigns.js:383-384/891-892` behaviour instead of silently overriding). Backend `normaliseStep1(arr, waveLabel)` helper in `/build` called for both waves BEFORE template validation → closes the gap where a direct API caller (or UI regression) could POST offending step-1 values and land step 1 five days late. Collapsible info box on `#wizardTab` (own localStorage key). Verified via STEP 7 (offending values on both waves → assert `delay_hours=0 AND condition='all'` on all step-1 rows post-job). Full narrative: `docs/sessions/DEPLOY_SIEMA_BATCH_20260903.md §2 (81c9a0f)`.

- [x] **Wizard cross-campaign duplicate detection** — commit `72a29d4`. Bug: `campaignBuilder.js:912` `ON CONFLICT (campaign_id, email) DO NOTHING` is per-campaign, and the four /segment prefetch sets never consulted `campaign_recipients`; two wizard runs against the same target expo enrolled the same email in both campaigns, shipping step 1 of BOTH. Fix, warning-first (never automatic): 5th prefetch query bounded by `LOWER(TRIM(cr.email)) = ANY($3)` (EXPLAIN: 0.096 ms on expo 9, 44 ms worst-case on expo 7); response gains `already_in_another_campaign` + `excluded_already_in_campaign` inside `counts` + `other_campaigns` alongside; UI amber warning row + "Remove them from this campaign" button re-runs `/segment` with `exclude_already_in_campaign=true` and adopts the new preview_token (server-side cache holds filtered lists → orchestrator's Phase 2 mint never sees excluded rows); STEP 8 in `test_wizard_silent.js` locks byte-identical detection-only counts + full-collapse-under-exclusion. Drafts count in overlap per Suer's Sunday-build / Monday-launch pattern (`cr.status='active'`, no `ec.status` predicate). Verified end-to-end 3 Sep: STEP 8 passed live; browser click-through on expo 11 → expo 17 showed 4→1 mailable transition after clicking Remove (server-side list changed, not just display); 27/27 wizard IDs present exactly once on deployed page; ~29 s Render 502 window. Two known limits recorded: overlap scoped to same target expo only (§9.1, deferred with a P2), preview-time race with no lock (§9.2, mitigated by re-Preview immediately before Build). Full narrative: `docs/sessions/DEPLOY_CROSS_CAMPAIGN_DEDUP_20260903.md`.

- [x] **CAN-SPAM footer + wizard preview readability** — commit `3f4da63` + docs `31c1267`. Morocco entity address (`ELAN EXPO MAROC SARL / 30, Bd Rahal El Meskini, 2ème Etage, Appart N° 5, Casablanca, Morocco / +212 650 219 756`) baked into the auto-appended unsub footer at `utils/trackingPixel.js:injectUnsubscribeLink` — campaign-only injection point (grep proof: exactly 1 call site at `email_worker.js:596`). UTF-8 preserved (verified end-to-end: `2ème` bytes `c3 a8`, `N°` bytes `c2 b0`, rendered intact in Gmail). Wizard preview restructured: headline block (Will receive / Excluded / Tokens to mint), G1 amber note (not struck-through), register-wave-empty state, collapsible Details ledger — all 21 pre-existing `wPv*` IDs preserved. Pre-deploy 21:02 baseline (21,389 / 2 / 93 / 21,294 / 0) + post-deploy 21:18 baseline (headline "21,286 will receive", "103 excluded (101 + 2)", "21,286 to mint") both verbatim in the deploy doc; arithmetic invariants held both times (drift is G1 only, +8 new SIEMA registrations in ~15 min). Full Gmail Authentication-Results header proves SPF pass via `em5759.leena.app` bounce subdomain — supersedes the SIEMA pre-launch audit's §C.1 apex-SPF finding (amended in place, withdrawn from ops checklist). Full narrative: `docs/sessions/DEPLOY_FOOTER_AND_WIZARD_POLISH_20260902.md`.
