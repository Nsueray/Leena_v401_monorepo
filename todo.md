...
| # | item | pri | note |
|---|---|---|---|
| 0 | **RUN `tests/test_activate_phone_smoke.js` TODAY** (Suer, TEST_JWT) | **P1 BLOCKER** | **BEFORE the wizard** — the `/activate` fail-open + A-1 `custom_fields` JSONB trace path shipped on 2 Sep in commit 9d868e3 but has NEVER been exercised end-to-end. Test creates 5 tokens on expo 17 via silent create-from-excel, activates row 4 (Turkey local → happy-path E.164) and row 3 (`12ab` → fail-open + trace), asserts stored phone + `custom_fields` for both. **Must pass before the SIEMA campaign mints real tokens** — if the trace INSERT is malformed, every activated reactivation visitor with a bad phone lands with a broken `custom_fields` blob. Run today, then cleanup SQL emitted by the test. |
